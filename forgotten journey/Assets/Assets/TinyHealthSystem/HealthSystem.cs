...
public class HealthSystem : MonoBehaviour
{
    ...
    {
        // Null 체크: UI 컴포넌트가 연결되어 있지 않은 경우 오류 방지
        if (currentHealthBar != null) UpdateHealthBar();
        if (currentHealthGlobe != null) UpdateHealthGlobe();
        if (currentManaBar != null) UpdateManaBar();
        if (currentManaGlobe != null) UpdateManaGlobe();
    }
    ...
    IEnumerator PlayerHurts()
    {
        ...
        // PopupText.Instance.Popup("Ouch!", 1f, 1f); // 원본 코드의 데모 팝업 제거
    ...
    IEnumerator PlayerDied()
    {
        ...
        // PopupText.Instance.Popup("You have died!", 1f, 1f); // 원본 코드의 데모 팝업 제거
